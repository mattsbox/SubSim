...
static gboolean handle_keys(GtkWidget *canvas,GdkEventKey *event,gpointer data)
{
	...
	switch(event->keyval)
	{
		...
		case GDK_KEY_Page_Up:
			//Projection mode sets the coordinate system to be relative to the view
...
void gldraw(void)
{
	...
	iterate();
	//Tells OpenGL to check the depth of objects to keep them from simply being drawn in order
...
static gboolean gtkdraw(GtkWidget *canvas,GdkEventExpose *event,gpointer user_data)
{
	...
	//Deferrs to the above method to handle creating the scene. This separates the two major technologies at use, GTK and OpenGL
	gldraw();
...
{
	//Gets the widget, which is known to be the GTKGLExt widget since it is the only widget
	GtkWidget *canvas = GTK_WIDGET(data);
	//Labels the widget's area invalid to indicates that it should be updated. &canvas->allocation retrieves that area
	gdk_window_invalidate_rect(canvas->window,&canvas->allocation,FALSE);
	//Generates the expose-event that triggers the above method, since the widget's area has been labeled invalid
...
static gboolean resize_window(GtkWidget *canvas,GdkEventConfigure *event, gpointer data)
{
	...
	glLoadIdentity();
	//Specifies the lower left corner, (0,0) and the width and height of the GTKGLExt widget so that OpenGL can convert them to screen coordinates
	glViewport(0,0,w,h);
	//Specifies the angle of the field of view of the camera in degrees, the aspect ratio and the near and far clipping planes for the scene
	gluPerspective(45,w/h,1,1000);
	//Shifts the coordinate system away from the viewer to give a good perspective. The original translation is -4.0f
...
int main(int argc, char **argv)
{
	...
	//Sets the handle_keys function to be called whenever a key is pressed or released, without sending any additional information
	...
	//Sets the proper callback functions for when closing the window is proposed and executed
	...
	//Sets the proper OpenGL settings inside the configuration object, allowing for depth testing, double buffering and RGBA coloring
	glConfiguration=gdk_gl_config_new_by_mode(GDK_GL_MODE_DEPTH|GDK_GL_MODE_DOUBLE|GDK_GL_MODE_RGBA);
	//Applies the settings of the configuration object to the GTKGLExt widget
	gtk_widget_set_gl_capability(canvas,glConfiguration,NULL,TRUE,GDK_GL_RGBA_TYPE);
	//Sets the GTKGLExt widget to call the gtkdraw function whenever it becomes aware that its area needs to be updated
	g_signal_connect(canvas,"expose-event",G_CALLBACK(gtkdraw),NULL);
	//Sets the resize_window function to handle any changes in the window dimensions
	g_signal_connect(canvas,"configure-event",G_CALLBACK(resize_window),NULL);
	//Shows all of the widgets in the window, which is simply the GTKGLExt widget
	...
	//Sets the update_window method to be called 30 times per second to draw the next animation frame
